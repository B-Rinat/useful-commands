...
| <code>$ certbot renew --cert-name yourdomain.com</code> | Renew the Certificate for a Specific Domain(s): Once you know the "certificate name", use the `--cert-name` option to renew only that specific certificate. Note: one certificate name can include several different domains |
